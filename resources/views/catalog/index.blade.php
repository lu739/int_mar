...
@section('content')
    ...
    <section>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5 mt-8">
            ...
            <a href="{{filter_url($categoryItem)}}"
{{--            <a href="{{route('catalog', ['category' => $categoryItem->slug])}}"--}}
               class="@if($categoryItem->id === $category->id) pointer-events-none bg-purple @else bg-card @endif
                      p-3 sm:p-4 2xl:p-6 rounded-xl hover:bg-pink text-xxs sm:text-xs lg:text-sm text-white font-semibold"
    ...
    <section class="mt-16 lg:mt-24">
        ...
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-6 2xl:gap-8 mt-8">
            ...
            <aside class="basis-2/5 xl:basis-1/4">
                <form action="{{route('catalog', ['category' => $category])}}"
...
{{--                    <!-- Filter item -->
                    ...
                    <div>
                        <button type="submit" class="w-full !h-16 btn btn-primary">OK</button>
                    </div>
                    @if(request('filters') || $category->exists())
                        <div>
                            <a href="{{route('catalog')}}" class="w-full !h-16 btn btn-outline">Сбросить фильтры</a>
                        </div>
                    @endif
                </form>
            </aside>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2">
                            <a href="{{filter_url($category, ['view' => 'grid'])}}"
                               class="@if(is_catalog_view('grid')) pointer-events-none text-pink @else text-white @endif
                            ...
                            <a href="{{filter_url($category, ['view' => 'list'])}}"
                               class="@if(is_catalog_view('list')) pointer-events-none text-pink @else text-white @endif inline-flex items-center justify-center w-10 h-10 rounded-md bg-card hover:text-pink">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 52 52">
                        ...
                        <div class="text-body text-xxs sm:text-xs">Найдено товаров: {{$products->total()}}</div>
                    </div>
                    <div x-data="{sort: '{{filter_url($category, ['sort' => request()->get('sort')])}}'}" class="flex flex-col sm:flex-row sm:items-center gap-3">
                        <span class="text-body text-xxs sm:text-xs">Сортировать по</span>
{{--                        <form x-ref="sortForm" action="{{route('catalog', ['category' => $category])}}">--}}
                            <select
                                name="sort"
                                x-model="sort"
                                ...
                                <option @selected(request('sort') === 'title') value="{{filter_url($category, ['sort' => 'title'])}}" class="text-dark">наименованию</option>
                            </select>
{{--                        </form>--}}
                    </div>
                </div>
                ...
                @if(is_catalog_view('list'))
                    <div class="products grid grid-cols-1 gap-x-2 gap-y-3">
