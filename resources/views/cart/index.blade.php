                        ...
                        <td scope="row" class="py-4 px-4 md:px-6 rounded-l-2xl bg-card">
                            <div class="flex flex-col lg:flex-row min-w-[200px] gap-2 lg:gap-6">
                                <div class="shrink-0 overflow-hidden w-[64px] lg:w-[84px] h-[64px] lg:h-[84px] rounded-2xl">
                                    <img src="{{'/storage/images/products/'.$cartItem->product->thumbnail}}" class="object-cover w-full h-full" alt="SteelSeries Aerox 3 Snow">
                                </div>
                                <div class="py-3">
                                    <h4 class="text-xs sm:text-sm xl:text-md font-bold">
                                        <a href="{{route('product.show', $cartItem->product->slug)}}" class="inline-block text-white hover:text-pink">{{$cartItem->product->title}}</a></h4>
...
{{--                                    </ul>--}}
                        ...
                        <td class="py-4 px-4 md:px-6 bg-card">
                            <div class="flex items-stretch h-[56px] gap-2">
                                <form action="{{route('cart.quantity', $cartItem->id)}}" method="post">
                                    @csrf
                                    <input type="hidden" name="quantity" value="-1">
                                    <button type="submit" class="w-12 h-full rounded-lg border border-body/10 hover:bg-card/20 active:bg-card/50 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition">-</button>
                                </form>
                                <form id="plusOneQuantity" action="{{route('cart.quantity', $cartItem->id)}}" method="post" class="order-1">
                                    @csrf
                                    <input type="hidden" name="quantity" value="1">
                                </form>
                                <form action="{{route('cart.quantity', $cartItem->id)}}" method="post" class="flex gap-2">
                                    @csrf
                                    <input class="h-full px-2 lg:px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition"
                                           name="new_total_quantity" type="number" min="1" max="999" value="{{$cartItem->quantity}}" placeholder="К-во">
                                    <button type="submit" form="plusOneQuantity" class="w-12 h-full rounded-lg border border-body/10 hover:bg-card/20 active:bg-card/50 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition">+</button>
                                </form>
                            </div>
                        </td>
                        <td class="py-4 px-4 md:px-6 bg-card">
                            <div class="font-medium whitespace-nowrap">{{$cartItem->amount}}</div>
                        </td>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mt-8">
            ...
            <div class="flex flex-col sm:flex-row lg:justify-end gap-4">
                ...
                <a href="{{route('order.checkout')}}" class="btn btn-purple">Оформить заказ</a>
            </div>
        </div>
